...
public class doubleInplaceCoverageTests
{
    ...
    public struct SetAllJob : IJob
    {
        public void Execute()
        {
            ...
            int n = 7;
            doubleN vec = arena.doubleVec(n);
            vec.setAll(3.5f);
            for (int i = 0; i < n; i++)
                Assert.AreEqual((double)3.5f, vec[i]);

            doubleMxN mat = arena.doubleMat(3, 4);
            mat.setAll(-2f);
            for (int i = 0; i < mat.Length; i++)
                Assert.AreEqual((double)(-2f), mat[i]);
    ...
    public struct CopyInplJob : IJob
    {
        public void Execute()
        {
            ...
            int n = 8;
            doubleN src = arena.doubleVec(n);
            doubleN dst = arena.doubleVec(n);
            for (int i = 0; i < n; i++)
            {
                src[i] = (double)(i + 1);
                dst[i] = 0;
    ...
    public struct EqualsByValueNegativeJob : IJob
    {
        public void Execute()
        {
            ...
            int n = 5;
            doubleN a = arena.doubleVec(n);
            doubleN c = arena.doubleVec(n);
            doubleN d = arena.doubleVec(n + 1); // length-mismatched, zeroed
            for (int i = 0; i < n; i++)
            {
                a[i] = (double)(i + 0.25f);
                c[i] = (double)(i + 0.25f);
            }
            c[2] = (double)999f; // diverge in one element
    ...
    public struct AlmostEqualsByValueJob : IJob
    {
        public void Execute()
        {
            ...
            int n = 5;
            doubleN a = arena.doubleVec(n);
            doubleN b = arena.doubleVec(n);
            doubleN c = arena.doubleVec(n);
            doubleN d = arena.doubleVec(n + 1);
            for (int i = 0; i < n; i++)
            {
                a[i] = (double)1.0f;
                b[i] = (double)1.0001f;   // within 1e-3
                c[i] = (double)1.01f;     // outside 1e-3
            }

            Assert.IsTrue (a.AlmostEqualsByValue(b, (double)1e-3f));
            Assert.IsFalse(a.AlmostEqualsByValue(c, (double)1e-3f));
            Assert.IsFalse(a.AlmostEqualsByValue(d, (double)1e-3f));
    ...
    public struct DotInplMatrixTransAJob : IJob
    {
        public void Execute()
        {
            ...
            doubleMxN A = arena.doubleMat(3, 2);
            A[0,0]=1; A[0,1]=2;
            A[1,0]=3; A[1,1]=4;
            A[2,0]=5; A[2,1]=6;

            doubleMxN B = arena.doubleMat(3, 4);
            for (int r = 0; r < 3; r++)
                for (int c = 0; c < 4; c++)
                    B[r, c] = (double)(r * 4 + c + 1);

            doubleMxN C = arena.doubleMat(2, 4);
            // garbage init, dotInpl must zero-fill before MAC
            for (int i = 0; i < C.Length; i++) C[i] = (double)999f;
            ...
            Assert.AreEqual((double)61f,  C[0, 0]);
            Assert.AreEqual((double)70f,  C[0, 1]);
            Assert.AreEqual((double)79f,  C[0, 2]);
            Assert.AreEqual((double)88f,  C[0, 3]);
            Assert.AreEqual((double)76f,  C[1, 0]);
            Assert.AreEqual((double)88f,  C[1, 1]);
            Assert.AreEqual((double)100f, C[1, 2]);
            Assert.AreEqual((double)112f, C[1, 3]);
    ...
    public struct DotInplVectorTransAJob : IJob
    {
        public void Execute()
        {
            ...
            doubleN y = arena.doubleVec(2);
            y[0] = (double)999f; y[1] = (double)999f;

            y.dotInpl(A, x, transposeA: true);

            // y[0] = 1+3+5 = 9; y[1] = 2+4+6 = 12
            Assert.AreEqual((double)9f,  y[0]);
            Assert.AreEqual((double)12f, y[1]);
    ...
    public struct SetColJob : IJob
    {
        public void Execute()
        {
            ...
            doubleMxN M = arena.doubleMat(3, 3);
            M[0,0]=1; M[0,1]=2; M[0,2]=3;
            M[1,0]=4; M[1,1]=5; M[1,2]=6;
            M[2,0]=7; M[2,1]=8; M[2,2]=9;

            doubleN newCol = arena.doubleVec(3);
            newCol[0] = (double)10f; newCol[1] = (double)20f; newCol[2] = (double)30f;

            M.SetCol(newCol, colIdx: 0, rowStartIdx: 0);

            // column 0 was overwritten...
            Assert.AreEqual((double)10f, M[0, 0]);
            Assert.AreEqual((double)20f, M[1, 0]);
            Assert.AreEqual((double)30f, M[2, 0]);
            // ...but other columns are untouched.
            Assert.AreEqual((double)2f, M[0, 1]);
            Assert.AreEqual((double)8f, M[2, 1]);
    ...
    public struct InplNoAllocContractJob : IJob
    {
        public void Execute()
        {
            ...
            doubleN b = arena.doubleVec(n);
            doubleN c = arena.doubleVec(n);
            doubleMxN M1 = arena.doubleMat(4, 4);
            doubleMxN M2 = arena.doubleMat(4, 4);
            doubleMxN Mout = arena.doubleMat(4, 4);
            ...
            doubleN x = arena.doubleVec(4);
            doubleN y = arena.doubleVec(4);
            for (int i = 0; i < 4; i++) x[i] = (double)(i + 1);
            ...
            a.setAll(1f);
            a.addInpl((double)0.5f);
            a.mulInpl((double)2f);
            a.divInpl((double)2f);
            a.subInpl((double)0.5f);
            a.modInpl((double)10f);
            a.signFlipInpl();
