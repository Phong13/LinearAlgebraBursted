...
public class TestNumericsOPsfProxy
{
    static void ToDotNetMatrix(fProxyMxN m, out Matrix<double> mm)
    {
    ...
    static void ToDotNetVector(fProxyN m, out Vector<double> mm)
    {
    ...
    static void DoLinAlgBurstedSVD(double[,] _A, out Vector<double> S, out Matrix<double> U, out Matrix<double> VT)
    {
        ...
        {
            fProxyMxN A = arena.fProxyMat(_A);
            ...
            var _tmp = fProxyOP.dot(w, vt);
            var AA = fProxyOP.dot(u, _tmp);
    ...
    static void DoLinAlgBurstedSVDSolve(double[,] _A, double[,] _b, out Matrix<double> x, fProxy eps)
    {
        Arena arena = new Arena(Unity.Collections.Allocator.Persistent);
        {
            fProxyMxN A = arena.fProxyMat(_A);
            fProxyMxN b = arena.fProxyMat(_b);
    ...
    public void TestMulMatMat()
    {
        ...
        Arena arena = new Arena(Unity.Collections.Allocator.Persistent);
        {
            fProxyMxN A = arena.fProxyMat(_A);
            fProxyMxN B = arena.fProxyMat(_B);
            fProxyMxN CCC = fProxyOP.dot(A, B);
